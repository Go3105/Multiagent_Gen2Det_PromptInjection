...
# Define agent states
class GenerateTeamState(TypedDict):
...
class CreateMultiAgent:
    def __init__(self, temperature, model_path, n_ctx, n_gpu_layers, n_batch, max_tokens, n_threads):
        ...
        self.workflow = StateGraph(GenerateTeamState)
    ...
    def create_supervisor_agent(self, state:GenerateTeamState):
    ...
    def agent_node(state:GenerateTeamState, agent, name):
...
class CreateMultiAgent:
    ...
    def graph(self, input, output):
        ...
        #写真を保存
        #display(Image(graph.get_graph().draw_mermaid_png(output_file_path="GenerateTeam_workflow.png")))
